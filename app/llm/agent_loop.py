    ...
    def _build_system_message(self, context_vars: dict, supplement: str = "") -> str:
        ...
        if context_vars.get("STEP_NAME") == "master":
            return "\n".join([
                ...
                "Version allocation only returns a future run_context_path; it does not create the file.",
                "A run_selected started/update event may precede persistence and carries no completion claim.",
                "Complete master steps 0.4–0.8, persist the full frozen context, and verify workspace readback.",
                "Only then report_progress with",
