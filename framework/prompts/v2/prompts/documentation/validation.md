...
cross-validation stage in `cross_validation/instructions.md`, NOT by the documentation step. Documentation may still run when it
...
`shared/state_contract.md`; documentation records the failure and never performs that regeneration.
...
- `metric_views/resolved_metric_view_capabilities.yaml` owns the run's Metric View capability and fallback decisions, including `PLATFORM` versus `ACCELERATOR` scope.
...
The README MUST have ALL of these sections from `documentation/instructions.md`:
