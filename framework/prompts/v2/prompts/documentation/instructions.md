...
> **Always load for this stage:** `shared/global_guardrails.md`, `documentation/validation.md`, `documentation/guardrails.md`, `shared/state_contract.md`.
> **Failure-only:** authenticate `run_context.inputs.stage_runbooks.generate_documentation` and load only the matching section of `documentation/runbook.md` after a classified failure. Never load the runbook on the normal success path.


## Failure-Only Runbook Loading

Do not read `documentation/runbook.md` during normal execution. After this stage classifies a failure, authenticate the frozen runbook tuple, use the routing index in `documentation/validation.md`, and load only the matching diagnostic section. The runbook cannot weaken a gate, change the failure owner, or authorize a blind retry.
...
This step uses **artifact-as-state** checkpointing (see `shared/state_contract.md`).
...
5. Manage `run_context.yaml` per `shared/state_contract.md` Section 8.
...
| generate_documentation | `documentation/readme.md` | canonical file and paired draft have exact current run/lifecycle/path, checkpoint, evidence-scope, and Genie quality-policy binding |
...
metric_views/resolved_metric_view_capabilities.yaml
...
**DO NOT:**
- Run Python code to load `gate_checks.py` or execute a cross-validation sweep during the documentation step. The sweep is the sole responsibility of the authenticated `cross_validation/instructions.md` stage, not the documentation stage.
...
| Metric View feature decision | `metric_views/resolved_metric_view_capabilities.yaml` and its recorded version/hash | verified generation and readback result |
...
**Reason:** <reason from metric_views/metric_view_plan.yaml>

**Capability:** <capability key; PLATFORM or ACCELERATOR scope; contract version/hash>

**Status:** NOT_IMPLEMENTED — documentation only

<sql block with the full reference SQL from metric_views/metric_view_plan.yaml; label its validation status from evidence>

**To implement manually:** <manual_implementation_notes from metric_views/metric_view_plan.yaml>
```
...
| Metric Layer | KPI reconciliation | PASS | VERIFIED | `metric_views/metric_view_validation.yaml` readback-backed checks |
| Metric Layer | Join fanout checks | PASS | VERIFIED | `metric_views/metric_view_validation.yaml` executed checks |
...
- Skipped or not-implemented KPIs from `metric_views/metric_view_validation.yaml`, reconciled with `metric_views/resolved_metric_view_capabilities.yaml` (with policy scope and reasons)
...
Do NOT:
- Re-read `erd_parsed.yaml`, `semantic_model.yaml`, `metric_views/metric_view_design.yaml`, manifests, etc.
