...
- A manifest supplies only the asset locator/deployment-attempt record. It is eligible for a deployment-phase skip only together with the separate matching readback-backed validation required by `shared/state_contract.md`.
...
`metric_views/metric_view_plan.yaml` MUST contain exactly one
