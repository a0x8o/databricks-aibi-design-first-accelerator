...
> **Always load for this stage:** `shared/global_guardrails.md`, `genie/validation.md`, `genie/guardrails.md`, `shared/state_contract.md`, `shared/sql_generation_rules.md`.
> **Failure-only:** authenticate `run_context.inputs.stage_runbooks.create_genie_space` and load only the matching section of `genie/runbook.md` after a classified failure. Never load the runbook on the normal success path.
...
Do not read `genie/runbook.md` during normal execution. After this stage classifies a failure, authenticate the frozen runbook tuple, use the routing index in `genie/validation.md`, and load only the matching diagnostic section. The runbook cannot weaken a gate, change the failure owner, or authorize a blind retry.
...
3. Manage `run_context.yaml` per `shared/state_contract.md` Section 8.
