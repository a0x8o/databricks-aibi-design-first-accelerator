...
> **Always load for this stage:** `shared/global_guardrails.md`, `metric_views/validation.md`, `metric_views/guardrails.md`, `shared/state_contract.md`, `shared/sql_generation_rules.md`.
> **Failure-only:** authenticate `run_context.inputs.stage_runbooks.create_metric_views` and load only the matching section of `metric_views/runbook.md` after a classified failure. Never load the runbook on the normal success path.
...
Do not read `metric_views/runbook.md` during normal execution. After this stage classifies a failure, authenticate the frozen runbook tuple, use the routing index in `metric_views/validation.md`, and load only the matching diagnostic section. The runbook cannot weaken a gate, change the failure owner, or authorize a blind retry.
...
`shared/state_contract.md`. The strategy-specific checkpoint is reusable only with that separate
...
Uses the fingerprinted checkpoint contract in `shared/state_contract.md`. Recompute the current
