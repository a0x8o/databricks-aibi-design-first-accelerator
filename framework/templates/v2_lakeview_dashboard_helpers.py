...
def configure_runtime(workspace_client, gate_module, store, expected_by_name):
    global w, _store, _expected_by_name, _gate_checks_loaded
    global run_dashboard_predeploy_gates, validate_dashboard_from_api, write_ground_truth_validation, GateCheckError
    if gate_module.CONTRACT_VERSION != "V2_PORTABLE_1":
        raise RuntimeError("DASHBOARD_HELPER_CONTRACT_ERROR: gate module version mismatch")
    w, _store, _expected_by_name = workspace_client, store, expected_by_name
    run_dashboard_predeploy_gates = gate_module.run_dashboard_predeploy_gates
    validate_dashboard_from_api = gate_module.validate_dashboard_from_api
    write_ground_truth_validation = gate_module.write_ground_truth_validation
    GateCheckError = gate_module.GateCheckError
...
def list_dashboards(page_size: int = 100, max_pages: int = 50) -> list[dict]:
    ...
    for _ in range(max_pages):
        ...
        if not page_token:
            break
    if page_token:
        raise RuntimeError('Dashboard listing incomplete: pagination limit reached')
    return out
...
def deploy_dashboard(
    ...
    *,
    ...
    """
    if not _gate_checks_loaded or display_name not in _expected_by_name:
        raise RuntimeError("DASHBOARD_HELPER_CONTRACT_ERROR: configure bound runtime and expected design first")
    sd = build_serialized_dashboard(datasets, pages, filter_dimensions)
    expected = _expected_by_name[display_name]
    if expected["serialized_dashboard"] != sd:
        raise RuntimeError("DASHBOARD_HELPER_CONTRACT_ERROR: compiled design differs from expected contract")
    run_dashboard_predeploy_gates(sd, display_name, required_artifacts=required_artifacts or [],
        quality_gates=quality_gates, store=_store)
    matches = [d for d in list_dashboards() if (d.get("display_name") or "").casefold() == display_name.casefold()]
    if len(matches) > 1:
        raise RuntimeError("DASHBOARD_HELPER_CONTRACT_ERROR: ambiguous dashboard title")
    dashboard_id = matches[0]["dashboard_id"] if matches else None
    ...
    if _gate_checks_loaded:
        try:
            post_deploy_result = validate_dashboard_from_api(
                w, dashboard_id, display_name,
                quality_gates=quality_gates, expected=expected,
            )
            if output_folder:
                ...
                write_ground_truth_validation(
                    validation_path, post_deploy_result, source="api_readback", store=_store,
                )
